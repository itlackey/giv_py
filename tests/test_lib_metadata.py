...
""")
            ...
            with change_directory(project_root):  # Application will work from repository root
                version = ProjectMetadata.get_version(commit="--current")
...
""")
            ...
            with change_directory(project_root):
                assert ProjectMetadata.get_title(commit="--current") == "complex-project"
...
""")
            ...
            with change_directory(project_root):
                assert ProjectMetadata.get_title(commit="--current") == "complex-node-app"
...
""")
    ...
    def test_monorepo_structure(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            subprocess.run(["git", "config", "user.name", "Test User"], cwd=monorepo, check=True)
